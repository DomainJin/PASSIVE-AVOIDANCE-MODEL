...
while cap.isOpened():
    ...
    if results and len(results) > 0:
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            for i, box in enumerate(boxes):
                ...
                if class_id == 1:  # 'lab-mouse' là class 1
            ...
            if mouse_detected:
                ...
                print(f"Frame {frame_count}/{total_frames} | Thời gian: {current_time:.2f}s | Lab Mouse: {lab_mouse}")
            else:
                print(f"Frame {frame_count}/{total_frames} | Thời gian: {current_time:.2f}s | Không phát hiện chuột")
    ...
    cv2.imshow("Mouse Tracking", annotated_frame)
...
# Hiển thị số lượng frame trong mỗi vùng
counts = [
...
# In ra thống kê chỉ cho những mask có giá trị
total_active = sum(counts[1:])  # Không tính vùng không xuất hiện
print("\nThống kê phân loại khu vực (frames):")

# Định nghĩa tên và khoảng frame tương ứng với mỗi mask
mask_ranges = [
    ("Không xuất hiện", 0),
    ("1-2 frames", counts[1]),
    ("3-29 frames", counts[2]),
    ("30-89 frames", counts[3]),
    ("90-149 frames", counts[4]),
    ("150-209 frames", counts[5]),
    ("210-269 frames", counts[6]),
    ("270-299 frames", counts[7]),
    ("≥300 frames", counts[8])
]

# In thống kê chỉ khi counts > 0
for name, count in mask_ranges[1:]:  # Bỏ qua "Không xuất hiện"
    if count > 0:
        print(f"- {name}: {count} khu vực ({count/total_active*100:.1f}% khu vực active)")

print(f"- Tổng khu vực active: {total_active}")

# Lưu hình ảnh phân loại chi tiết
plt.savefig("detailed_mouse_classification.png", dpi=300, bbox_inches='tight')
print("Đã lưu phân loại chi tiết vào detailed_mouse_classification.png")
